...
function createState(dateKey: string): DailyActivityState {
  return {
    dateKey,
    words: [],
    stories: [],
...
export class DailyActivityModule {
  ...
  private readonly wordsTaskEl: HTMLElement;
  private readonly storyTaskEl: HTMLElement;
  private readonly interactionTaskEl: HTMLElement;
  private readonly dateEl: HTMLElement;
  private state: DailyActivityState;

  constructor(rootEl: HTMLElement) {
    const summaryEl = rootEl.querySelector<HTMLElement>('#daily-activity-summary');
    const wordsTaskEl = rootEl.querySelector<HTMLElement>('#daily-task-words');
    const storyTaskEl = rootEl.querySelector<HTMLElement>('#daily-task-story');
    const interactionTaskEl = rootEl.querySelector<HTMLElement>('#daily-task-interaction');
    const dateEl = rootEl.querySelector<HTMLElement>('#daily-activity-date');
  ...
  trackStory(sentence: string): void {
    this.ensureCurrentDate();
  ...
  private loadState(): DailyActivityState {
    const todayKey = this.getTodayKey();
    ...
    try {
      const parsed = JSON.parse(raw) as Partial<DailyActivityState>;
