...
export class TouchGameModule {
  ...
  private targetSequence: VocabularyWord[] = [];
  private targetSequenceIndex = 0;
  ...
  private getInitialTargetButton(): HTMLButtonElement | null {
    if (this.targetSequence.length === 0) {
      this.resetTargetSequence();
    }

    const wordId = this.targetSequence[this.targetSequenceIndex];
    return this.findButtonByWordId(wordId) ?? this.gridEl.querySelector<HTMLButtonElement>('.word-card');
  }
  ...
  private resetTargetSequence(avoidWord?: VocabularyWord): void {
  ...
  private findButtonByWordId(wordId: VocabularyWord | undefined): HTMLButtonElement | null {
    if (!wordId) {
      return null;
    }
    ...
    return (
      Array.from(this.gridEl.querySelectorAll<HTMLButtonElement>('.word-card')).find(
        (button) => button.dataset.wordId === wordId
      ) ?? null
    );
  }
